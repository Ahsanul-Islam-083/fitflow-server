// Server dependencies and environment configuration
...
// Express settings and CORS configuration
...
// MongoDB client configured for a stable server API version
...
// Requests must provide a valid Bearer token signed by the remote JWKS endpoint.
...
const verifyTrainer = async (req, res, next) => {
  // Require trainer or admin role for trainer-specific routes
  const user = req.user;
...
const verifyNotBlocked = async (req, res, next) => {
  // Blocked users cannot perform certain write actions
  const user = req.user;
  try {
    const dbUser = await usersCollection.findOne({ email: user.email });
...
// Small ping check for MongoDB connectivity during startup
async function run() {
  try {
...
// Database collections used across the API
const db = client.db("FitFlow");
// Collection references for domain data
...
// Helper functions to create notification documents for admins and users
...
// Forum post routes
// Add a new forum post by an authenticated, non-blocked user
...
// Authorization middleware for forum post ownership
// Ensures only the post author or an admin can update or delete a post
...
// Authorization middleware for comment ownership
// Ensures only the comment author or an admin can update or delete a comment
...
// Apply to become a trainer and notify admins for review
...
// Create a new class by an authenticated trainer or admin
...
// Booking routes and collection reference
// Stores class booking and payment records for users and trainers
const bookingsCollection = db.collection("bookings");
...
// Attach corresponding user information to each booking record
...
// Attach detailed class information to each booking record
...
// Helper to filter bookings, compute booking stats, and paginate results
...
// Booking routes
// Get all bookings for admin users with optional pagination and stats
...
// Favorite class routes
// Get all favorite class IDs for a specific user
